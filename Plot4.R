...
pos1<-subset(SCC,grepl("[Cc]oal",Short.Name))
pos1<-pos1$SCC

data_coal<-NEI%>%filter(SCC %in% pos1)%>%group_by(type,year)%>%
...
##plot the graph 
myplot4<-qplot(year,total,data = data_coal,geom=c("line","point"),color=type)+
             geom_smooth(method = "lm",se=FALSE)+
             labs(x="Year",y="PM 2.5 Emissions in Tons", 
                             title = "Decrease in Emissions by Coal Sources")
print(myplot4)
